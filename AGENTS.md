...
| Chat-scoped preview surface  | Image large view, markdown/text reading, read-only code, single-file diff     | Shared chrome: `src/mainview/components/primitives/ContentSurface.tsx`; target host: chat content stage in `src/mainview/layouts/MainLayout.tsx`; current state: `src/mainview/stores/use-chat-overlay-store.ts` | visible/compact  |
