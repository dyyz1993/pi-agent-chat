...
class TicTacToeComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      dim("\u2500".repeat(leftBorder)) + bold(blue(titleText)) + dim("\u2500".repeat(rightBorder)),
    );
    ...
    if (this.state.status !== "playing") {
      ...
      lines.push(
        centerPad(
          `Turn: ${bold(blue("X"))} (You)  ${dim("|")}  ${bold(yellow("O"))} (Agent)`,
          width,
        ),
      );
    } else {
      lines.push(
        centerPad(`${blue("X")} (You)  ${dim("|")}  Turn: ${bold(yellow("O"))} (Agent)`, width),
      );
    }
...
`;
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      ...
      return new Text(
        theme.fg("toolTitle", theme.bold("tic_tac_toe ")) + theme.fg("muted", action),
        0,
        0,
      );
    },
  ...
  pi.registerTool({
    ...
    renderResult(result, { expanded }, theme) {
      ...
      const summary =
        ...
        theme.fg(
          "muted",
          `cursor (${details?.agentCursorRow ?? 0},${details?.agentCursorCol ?? 0})`,
        );
      if (expanded && details) {
        return new BannerMessageComponent(summary, details, true, theme);
      }
