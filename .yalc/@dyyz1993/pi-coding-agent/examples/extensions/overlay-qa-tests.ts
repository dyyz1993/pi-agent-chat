...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("overlay-overflow", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new StreamingOverflowComponent(tui, theme, done),
        {
          overlay: true,
  ...
  pi.registerCommand("overlay-streaming", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ctx.ui.setEditorText("");
      await ctx.ui.custom<void>(
        (tui, theme, _kb, done) => new StreamingInputController(tui, theme, done),
        {
          overlay: true,
...
abstract class BaseOverlay {
  ...
  protected box(lines: string[], width: number, title?: string): string[] {
    ...
    result.push(
      th.fg("border", `╭${topLine}`) + th.fg("accent", titleStr) + th.fg("border", `${topLine2}╮`),
    );

    for (const line of lines) {
      result.push(
        th.fg("border", "│") + truncateToWidth(line, innerW, "...", true) + th.fg("border", "│"),
      );
    }
...
class StreamingOverflowComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    result.push(
      border("│") +
        padLine(canScrollUp || canScrollDown ? th.fg("dim", ` ${scrollInfo}`) : "") +
        border("│"),
    );
    ...
    result.push(
      border("│") + padLine(` ${status} ${th.fg("dim", "| ↑↓ scroll | Esc close")}`) + border("│"),
    );
...
class SidepanelComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(border("├") + border("─".repeat(innerW)) + border("┤"));
    lines.push(
      border("│") + padLine(th.fg("warning", " ⚠ Resize terminal < 100 cols")) + border("│"),
    );
...
class AnimationDemoComponent extends BaseOverlay {
  ...
  render(width: number): string[] {
    ...
    lines.push(border("│") + padLine(``) + border("│"));
    lines.push(
      border("│") + padLine(` Frame: ${th.fg("accent", String(this.frame))}`) + border("│"),
    );
    ...
    lines.push(border("│") + padLine(``) + border("│"));
    lines.push(
      border("│") + padLine(th.fg("dim", " This proves overlays can handle")) + border("│"),
    );
    lines.push(
      border("│") + padLine(th.fg("dim", " real-time game-like rendering.")) + border("│"),
    );
...
class FocusDemoController extends BaseOverlay {
  ...
  render(width: number): string[] {
    const th = this.theme;
    const focused =
      this.focusIndex === -1 ? "Controller" : (this.panels[this.focusIndex]?.label ?? "?");
    return this.box(
      [
        "",
