import { uuidv7 } from "@dyyz1993/pi-agent-core";
    ...
    const appendMessage = (entry) => {
        if (entry.type === "message") {
            ...
            if (entry.message.role === "assistant" &&
                ...
                const filteredContent = entry.message.content.filter((part) => !(part.type === "toolCall" && part.id !== undefined && strippedToolCallIds.has(part.id)));
                if (filteredContent.length !== entry.message.content.length) {
                    messages.push({ ...entry.message, content: filteredContent });
                    return;
                }
            }
...
export class SessionManager {
    ...
    _buildIndex() {
        ...
        for (let i = this.fileEntries.length - 1; i >= 0; i--) {
            ...
            if (targetId === null) {
                ...
                for (let j = i + 1; j < this.fileEntries.length; j++) {
                    ...
                    if (candidate.type === "session" || candidate.type === "leaf_pointer")
                        continue;
            ...
            if (this.byId.has(targetId)) {
                ...
                for (let j = i + 1; j < this.fileEntries.length; j++) {
                    ...
                    if (parentIds.has(candidate.id))
                        continue;
                    if (!this._isDescendantOf(candidate, targetId))
                        continue;
                    const depth = this._getEntryDepth(candidate);
                    if (depth > bestDepth) {
                        bestDepth = depth;
        ...
        for (const [id, entry] of this.byId) {
            if (entry.type === "leaf_pointer" || parentIds.has(id))
                continue;
    ...
    _isDescendantOf(entry, ancestorId) {
        let currentId = entry.parentId;
        const visited = new Set();
        while (currentId !== null && this.byId.has(currentId) && !visited.has(currentId)) {
    ...
    countUserMessagesOnPath(leafId) {
        let count = 0;
