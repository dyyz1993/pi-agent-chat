...
export class ExtensionRunner {
    ...
    _currentExtensionName = "";
    getProjectRootFn = () => this.cwd;
    getSessionDataDirFn = () => "";
    getProjectDataDirFn = () => "";
    getCwdDataDirFn = () => "";
    getGlobalDataDirFn = () => "";
    getFileSnapshotManagerFn = () => null;
    respondUIFn = () => () => { };
    ...
    setContextDirFns(fns) {
        ...
        if (fns.getSessionDataDir)
            this.getSessionDataDirFn = () => fns.getSessionDataDir(getExtName());
        if (fns.getProjectDataDir)
            this.getProjectDataDirFn = () => fns.getProjectDataDir(getExtName());
        if (fns.getCwdDataDir)
            this.getCwdDataDirFn = () => fns.getCwdDataDir(getExtName());
        if (fns.getGlobalDataDir)
            this.getGlobalDataDirFn = () => fns.getGlobalDataDir(getExtName());
    }
    setFileSnapshotManagerFn(fn) {
