...
export function isWithinSandboxSync(filePath, sandboxDir) {
    const rel = relative(sandboxDir, filePath);
    return !rel.startsWith("..") && !normalize(rel).startsWith("..");
}
async function realpathDeepestExisting(filePath) {
    let dir = filePath;
...
export async function isWithinSandbox(filePath, sandboxDir, options) {
...
export async function safeJoin(sandboxDir, filename, options) {
