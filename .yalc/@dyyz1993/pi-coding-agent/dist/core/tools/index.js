...
export { OutputCollector } from "./output-collector.js";
export { createReadTool, createReadToolDefinition, } from "./read.js";
export { spawnManagedProcess } from "./spawn-managed.js";
export { stripMarkdownCodeBlock } from "./strip-markdown.js";
export { DEFAULT_INPUT_MAX_BYTES, DEFAULT_MAX_BYTES, DEFAULT_MAX_LINES, formatSize, truncateHead, truncateLine, truncateTail, } from "./truncate.js";
...
export function toolsOptionsFromProvider(provider) {
    const options = {};
    if (provider.bash)
        options.bash = { operations: provider.bash };
    if (provider.read)
        options.read = { operations: provider.read };
    if (provider.write)
        options.write = { operations: provider.write };
    if (provider.edit)
        options.edit = { operations: provider.edit };
    if (provider.grep)
        options.grep = { operations: provider.grep };
    if (provider.find)
        options.find = { operations: provider.find };
    if (provider.ls)
        options.ls = { operations: provider.ls };
    return options;
}
