...
import { CombinedAutocompleteProvider, Container, fuzzyFilter, getCapabilities, hyperlink, Loader, Markdown, matchesKey, ProcessTerminal, Spacer, setKeybindings, Text, TruncatedText, TUI, visibleWidth, } from "@dyyz1993/pi-tui";
    ...
    async showModelsSelector() {
        ...
        if (hasSessionScope) {
            ...
            if (patterns !== undefined && patterns.length > 0) {
                const scopedModels = await resolveModelScope(patterns, this.session.modelRegistry, this.settingsManager.getTierModels());
        ...
        const updateSessionModels = async (enabledIds) => {
            ...
            if (enabledIds && enabledIds.length > 0 && enabledIds.length < allModels.length) {
                const newScopedModels = await resolveModelScope(enabledIds, this.session.modelRegistry, this.settingsManager.getTierModels());
