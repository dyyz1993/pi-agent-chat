...
async function runStructuredOutput(session, schema, initialMessage, initialImages, messages) {
    ...
    for (let attempt = 0; attempt <= MAX_STRUCTURED_RETRIES; attempt++) {
        ...
        if (attempt < MAX_STRUCTURED_RETRIES) {
            await session.prompt(`Your previous response was invalid: ${result.error}. Please respond with valid JSON matching the schema.`);
            continue;
        }
        console.error(`Structured output validation failed after ${MAX_STRUCTURED_RETRIES + 1} attempts: ${result.error}`);
        return 1;
    }
    return 1;
}
