...
import { discoverAgents } from "../../core/agent-types.js";
...
import { createBranchSummaryMessage } from "../../core/messages.js";
...
const PERMISSION_MODES = ["auto", "acceptEdits", "dontAsk", "always-allow", "always-deny"];
function isPermissionMode(mode) {
    return PERMISSION_MODES.includes(mode);
}
function getTreeEntryLabel(entry) {
    if ("message" in entry) {
...
function isSessionMessageEntry(entry) {
...
export async function runRpcMode(runtimeHost) {
    ...
    const channelManager = new ChannelManager((message) => {
        output(message);
    });
    ...
    function createDialogPromise(opts, defaultValue, request, parseResponse) {
        ...
        return new Promise((resolve, reject) => {
            let timeoutId;
            const cleanup = (reason = "responded") => {
                if (timeoutId)
    ...
    const rebindSession = async () => {
        ...
        await session.bindExtensions({
            ...
            registerChannel: (name) => channelManager.register(name),
        });
        unsubscribe?.();
    ...
    const handleCommand = async (command) => {
        ...
        switch (command.type) {
            ...
            case "set_tier_models": {
                ...
                for (const tierName of Object.keys(command.models)) {
                    if (!validTiers.has(tierName)) {
                        return error(id, "set_tier_models", `Invalid tier name: "${tierName}". Valid tiers are: fast, pro, max`);
                    }
                }
                const mapping = {};
                if (command.models.fast !== undefined)
                    mapping.fast = command.models.fast;
                if (command.models.pro !== undefined)
                    mapping.pro = command.models.pro;
                if (command.models.max !== undefined)
                    mapping.max = command.models.max;
            ...
            case "get_full_messages": {
                ...
                for (const entry of branchEntries) {
                    if (entry.type === "segment_summary") {
                        for (let index = 0; index < entry.targetIds.length; index++) {
                            ...
                            if (deletedIds.has(targetId) || segmentTargets.has(targetId))
                                continue;
                ...
                for (const entry of branchEntries) {
                    if (!isSessionMessageEntry(entry) || deletedIds.has(entry.id))
                        continue;
            ...
            case "get_modified_files": {
                ...
                if (command.toUserMsgEntryId) {
                    ...
                    const resolvedSnapshotEntryId = fileSnapshotManager.resolveSnapshotEntryIdForTarget(command.toUserMsgEntryId, entries);
                    return success(id, "get_modified_files", {
                        files,
            ...
            case "get_file_diff": {
                ...
                return success(id, "get_file_diff", diff
                    ? {
                        path: diff.path,
            ...
            case "get_batch_diffs": {
                ...
                return success(id, "get_batch_diffs", fileSnapshotManager.getBatchDiffs({
                    fromEntryId: command.fromEntryId,
                    toEntryId: command.toEntryId,
                }));
            }
            ...
            case "get_skills": {
                ...
                const rpcSkills = skills.map((skill) => ({
                    name: skill.name,
                    description: skill.description,
            ...
            case "get_extensions": {
                ...
                const rpcExtensions = extensions.map((extension) => ({
                    path: extension.path,
            ...
            case "get_tools": {
                const tools = session.extensionRunner.getAllRegisteredTools().map((tool) => ({
            ...
            // =================================================================
            case "get_settings": {
                const settings = command.scope === "project"
            ...
            case "get_flags": {
                const flags = Array.from(session.extensionRunner.getFlags().entries()).map(([name, flag]) => ({
                    name,
            ...
            case "set_permission_mode": {
                if (!isPermissionMode(command.mode)) {
                    return error(id, "set_permission_mode", `Invalid permission mode: "${command.mode}". Valid modes: ${PERMISSION_MODES.join(", ")}`);
                }
            ...
            case "get_mcp_servers": {
                ...
                const servers = manager.getConnections().map((conn) => ({
                    name: conn.name,
                    status: conn.status,
                    error: conn.error,
                    ...
                    scope: conn.name in serverConfigs ? "project" : "global",
    ...
    const handleInputLine = async (line) => {
        ...
        if (typeof parsed === "object" &&
            ...
            "name" in parsed) {
            channelManager.handleInbound(parsed);
            return;
        }
