...
import type { ExtensionFactory, ExtensionAPI, ExtensionContext, AgentHook, AgentHooks, AgentHookEntry } from "@dyyz1993/pi-coding-agent";
...
const factory: ExtensionFactory = (pi) => {
  ...
  const subscribe = (eventName: string) => {
    ...
    (pi as unknown as DynamicEventEmitter).on(eventName, async (event: Record<string, unknown>, ctx: ExtensionContext) => {
      ...
      const settingsHooks = cachedSettingsHooks[hookKey] ?? cachedSettingsHooks[eventName] ?? cachedSettingsHooks["*"] ?? [];
