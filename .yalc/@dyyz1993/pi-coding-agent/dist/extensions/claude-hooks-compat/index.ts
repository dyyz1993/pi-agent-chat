import { createTypedChannel, type ExtensionAPI, type ToolResultEvent, type BeforeAgentStartEvent, type ExtensionFactory } from "@dyyz1993/pi-coding-agent";
...
const factory: ExtensionFactory = (pi) => {
	...
	async function processHookEvent(
		...
		if (hookEventName === "PreToolUse") {
			const eventVars = (event as Record<string, unknown>).variables as Record<string, string> | undefined;
			const permissionMode = eventVars?.["permissionMode"];
			if (permissionMode === "dontAsk" || permissionMode === "always-allow") {
				return undefined;
			}
		}
