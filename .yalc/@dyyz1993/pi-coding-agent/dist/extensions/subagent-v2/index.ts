...
const SubagentParams = Type.Object({
  ...
  timeout: Type.Optional(Type.Number({ description: "Timeout in seconds. Default: 1800 (30 minutes).", default: 1800 })),
...
const factory: ExtensionFactory = (pi) => {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const timeoutMs = (params.timeout ?? 1800) * 1000;
        ...
        if (result.status === "timeout") {
          return {
            content: [
              {
                type: "text",
                text: result.finalText || "(no output captured)",
              },
            ],
