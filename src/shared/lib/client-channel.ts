...
type MethodKeys<T extends ChannelContract> =
  T["methods"] extends Record<string, unknown> ? keyof T["methods"] & string : string;

type MethodParams<T extends ChannelContract, K extends string> =
  T["methods"] extends Record<string, infer M>
    ? K extends keyof M
...
type MethodReturn<T extends ChannelContract, K extends string> =
  T["methods"] extends Record<string, infer M>
    ? K extends keyof M
...
export class ClientChannel<T extends ChannelContract = ChannelContract> {
  ...
  async call<K extends MethodKeys<T>>(
    method: K,
