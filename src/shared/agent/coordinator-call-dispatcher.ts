  ...
  try {
    switch (method) {
      ...
      case "session_delegate_send":
        result = await options.handleDelegateSend(options.sessionId, msg);
        break;
      ...
      case "session_delegate_status":
        result = await options.handleDelegateStatus(options.sessionId, msg);
        break;
      ...
      default:
        if (method === "session_delegate_clear_stopped") {
          result = options.handleClearStopped(options.sessionId, msg);
