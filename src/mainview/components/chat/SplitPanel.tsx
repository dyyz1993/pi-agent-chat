    ...
    >
      {children.map((child, i) => (
        <div key={i} style={{ flex: `0 0 ${sizes[i]}%`, overflow: "hidden" }}>
          ...
          {i < children.length - 1 && (
            <div
              className={`${isHorizontal ? "w-1 cursor-col-resize hover:bg-semantic-accent/40" : "h-1 cursor-row-resize hover:bg-semantic-accent/40"} bg-surface-hover/30 shrink-0 transition-colors touch-none select-none`}
