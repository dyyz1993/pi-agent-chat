...
const BG_MAP: Record<string, string> = {
  pending: "bg-amber-950/8 border border-amber-500/30",
  responded: "bg-emerald-950/6 border-l-2 border-emerald-500/30",
  dismissed: "bg-gray-950/5 border-l-2 border-gray-600/30",
  notified: "bg-cyan-950/6 border-l-2 border-cyan-500/30",
};
  ...
  return (
    <div className={`overflow-hidden rounded ${BG_MAP[block.status] ?? ""}`} data-ui-request-id={block.id}>
      <div className="px-3 py-1.5 pl-2 flex items-center gap-2 text-xs">
        ...
        {isPending && (
          <span className="text-amber-400 animate-pulse text-[10px] flex items-center gap-1">
      ...
      {block.message && (
        <div className="px-3 pb-1 text-[11px] text-gray-400 leading-relaxed">
          {block.message}
        </div>
      )}
      {children}
      ...
      {isPending ? (
        <div className="px-3 py-1.5 flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 py-1 text-[11px] rounded bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 py-1 text-[11px] rounded bg-red-600/15 text-red-400 hover:bg-red-600/25 transition-colors"
          >
        ...
        <div className="px-3 pb-1.5">
          <span
            className={`text-[11px] ${block.response?.confirmed ? "text-emerald-400" : "text-red-400"}`}
          >
            {responseText}
          </span>
        </div>
          ...
          <div className="px-3 py-2 space-y-0.5">
            ...
            <div className="flex items-center gap-2 px-2 py-1.5">
              <button
                ...
                className={`shrink-0 ${customSelected ? "text-sky-400" : "text-gray-600"}`}
              >
                {customSelected ? <CheckSquare className="w-3.5 h-3.5" /> : <Square className="w-3.5 h-3.5" />}
              </button>
              <span className={`text-[11px] ${customSelected ? "text-sky-300" : "text-gray-400"}`}>自定义答案</span>
            </div>
            {customSelected && (
              <input
                type="text"
                ...
                className="w-full ml-6 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-[11px] text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-amber-500/50"
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md bg-amber-600/20 text-amber-400 hover:bg-amber-600/30 disabled:opacity-40 disabled:cursor-not-allowed text-[11px] transition-colors"
              >
                提交
              </button>
              <button
        ...
        <div className="px-3 py-2 space-y-0.5">
          {options.map((opt, i) => {
            ...
            return (
              ...
              >
                <CircleDot className={`w-3.5 h-3.5 shrink-0 ${selectedIdx === i ? "text-sky-400" : "text-gray-600"}`} />
                <div className="min-w-0">
                  <div>{label}</div>
                  {desc && <div className="text-[10px] text-gray-500">{desc}</div>}
                </div>
          ...
          <div className="flex items-center gap-2 px-2 py-1.5">
            <button
              ...
              className={`shrink-0 ${customSelected || selectedIdx === -1 ? "text-sky-400" : "text-gray-600"}`}
            >
              {(customSelected || selectedIdx === -1) ? <CircleDot className="w-3.5 h-3.5 text-sky-400" /> : <CircleDot className="w-3.5 h-3.5 text-gray-600" />}
            </button>
            <span className={`text-[11px] ${customSelected || selectedIdx === -1 ? "text-sky-300" : "text-gray-400"}`}>自定义答案</span>
          </div>
          {customSelected && (
            <input
              type="text"
              ...
              onChange={(e) => setCustomValue(e.target.value)}
              placeholder={block.placeholder ?? "输入你的答案"}
              className="w-full ml-7 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-[11px] text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-amber-500/50"
              onKeyDown={(e) => e.key === "Enter" && customValue.trim() && respondById(block.id, { value: customValue.trim() })}
            />
          )}
          <div className="flex gap-2 pt-1">
      ...
      {isPending ? (
        <div className="px-3 py-1.5 flex gap-1.5">
          <input
            ...
            className="flex-1 bg-gray-800/60 border border-gray-700/50 rounded px-2 py-1 text-[11px] text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-amber-500/50"
            onKeyDown={(e) => {
          ...
          <button
            ...
            className="flex items-center justify-center px-2 py-1 rounded bg-amber-600/20 text-amber-400 hover:bg-amber-600/30 transition-colors"
          >
      ...
      {isPending ? (
        <div className="px-3 py-1.5 space-y-1.5">
          <textarea
            ...
            rows={4}
            className="w-full bg-gray-800/60 border border-gray-700/50 rounded px-2 py-1 text-[11px] text-gray-200 font-mono placeholder:text-gray-600 focus:outline-none focus:border-violet-500/50 resize-y"
          />
          <div className="flex gap-1.5">
            <button
              onClick={() => respondById(block.id, { value })}
              className="flex-1 flex items-center justify-center gap-1 py-1 text-[11px] rounded bg-violet-600/20 text-violet-400 hover:bg-violet-600/30 transition-colors"
            >
              <Send className="w-3 h-3" />
              提交
            </button>
            <button
              onClick={() => dismissById(block.id)}
              className="flex items-center justify-center gap-1 px-2 py-1 text-[11px] rounded bg-gray-600/15 text-gray-400 hover:bg-gray-600/25 transition-colors"
            >
              <X className="w-3 h-3" />
              取消
            </button>
        ...
        <div className="px-3 pb-1.5">
          <details>
            <summary className="text-[11px] text-violet-400 cursor-pointer hover:text-violet-300">
              编辑内容 ({responseValue.length} 字符)
            </summary>
            <pre className="mt-1 text-[11px] text-gray-300 bg-gray-800/40 rounded p-2 max-h-40 overflow-auto whitespace-pre-wrap font-mono">
              {responseValue}
            </pre>
          </details>
        </div>
        ...
        <div className="px-3 pb-1.5">
          <span className="text-[11px] text-gray-500">已取消编辑</span>
        </div>
      ) : null}
    </CardShell>
  );
...
export const RespondUICard = memo(function RespondUICard({
  ...
  return (
    <div className="overflow-hidden rounded bg-orange-950/6 border-l-2 border-orange-500/30" data-ui-request-id={block.id}>
      <div className="px-3 py-1.5 pl-2 flex items-center gap-2 text-xs">
        <Icon className={`w-3.5 h-3.5 shrink-0 ${color}`} />
        <span className={`font-medium ${color}`}>
          {block.title ?? "异步响应注入"}
        </span>
        <Zap className="w-3 h-3 text-orange-400 shrink-0 ml-auto" />
      </div>
      {block.message && (
        <div className="px-3 pb-1.5 text-[11px] text-orange-300/70">
          {block.message}
        </div>
      )}
    </div>
