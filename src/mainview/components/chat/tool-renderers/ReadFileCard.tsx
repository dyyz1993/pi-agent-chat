	...
	return (
		<div data-block-id={blockId} className={`border-x-0 border-t border-b overflow-hidden ${
			...
			<div className="px-3 py-1.5 flex items-center gap-2 text-xs">
				<FileText className={`w-3.5 h-3.5 shrink-0 ${isRunning ? "text-blue-400" : isError ? "text-red-400" : "text-blue-400/60"}`} />
				...
				{isRunning && <span className="ml-auto text-[10px] text-blue-400 animate-pulse shrink-0">reading</span>}
			</div>
			...
			<details className="group" open>
				...
				<div className="px-3 pb-2">
					{block.output ? (
						...
						<div className="text-[11px] text-gray-600 italic py-1">读取中...</div>
					) : null}
				</div>
			</details>

			{rulesData && rulesData.rulesMatched && rulesData.rulesMatched.length > 0 && (
				<details className="group border-t border-indigo-700/20">
					<summary className="px-3 py-1 text-[11px] text-indigo-400 cursor-pointer hover:text-indigo-300 select-none flex items-center gap-1.5">
						...
						<span className="text-indigo-600 ml-1">{rulesData.rulesMatched.length} rule{rulesData.rulesMatched.length !== 1 ? "s" : ""}</span>
					</summary>
					<div className="px-3 pb-2">
						{rulesData.rulesMatched.map((rule) => (
							<div key={rule.name} className="border-b last:border-b-0 border-indigo-700/10 py-1 flex items-center gap-1.5">
								<span className={`text-[10px] font-medium shrink-0 ${rule.severity === "critical" ? "text-red-400" : rule.severity === "high" ? "text-amber-400" : "text-indigo-300"}`}>{rule.title}</span>
								<span className="text-[10px] text-gray-600 font-mono">{rule.matchedGlob}</span>
							</div>
						))}
					</div>
				</details>
