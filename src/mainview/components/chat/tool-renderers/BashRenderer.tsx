...
function formatDuration(ms: number): string {
	...
	if (isBackground) {
		...
		statusLabel = <span className="text-yellow-600 dark:text-yellow-400 text-[10px]">已后台运行</span>;
	...
	return (
		<div data-block-id={blockId} className={`rounded-none overflow-hidden border-x-0 border-t border-b ${borderBg}`}>
			<div className="px-3 py-1.5 flex items-center gap-2 text-xs">
				<span className={`font-medium ${isBackground ? "text-yellow-600 dark:text-yellow-400" : isTerminated ? "text-red-500 dark:text-red-400" : isRunning ? "text-blue-500 dark:text-blue-400" : isError ? "text-red-500 dark:text-red-400" : "text-gray-800 dark:text-gray-300"}`}>{block.toolName}</span>
				{isRunning && !statusLabel && <span className="text-blue-500 dark:text-blue-400 animate-pulse text-[10px]">running</span>}
				{statusLabel}
			{bashDetails?.background && <span className="text-[10px] text-gray-400 dark:text-gray-500">PID: {bashDetails.background.pid}</span>}
			{(bashDetails?.background ?? (storeStatus === "background" && bashProcess)) && (
				<span className="text-[10px] text-gray-400 dark:text-gray-500">{formatDuration(bashDetails?.background?.durationMs ?? ((Date.now() - (bashProcess?.startedAt ?? Date.now()))))}</span>
			)}
			{(bashDetails?.terminated ?? (storeStatus === "terminated" && bashProcess)) && (
				<span className="text-[10px] text-gray-400 dark:text-gray-500">{formatDuration(bashDetails?.terminated?.durationMs ?? ((bashProcess?.endedAt ?? Date.now()) - (bashProcess?.startedAt ?? Date.now())))}</span>
			)}
			</div>
			...
			<details className="group">
				...
				<div className="px-3 pb-2">
					{block.args ? (
						<pre className="text-[11px] text-yellow-600/70 dark:text-yellow-300/60 overflow-x-auto whitespace-pre-wrap font-mono max-h-40 overflow-y-auto leading-relaxed">{tryFormatAsYaml(block.args)}</pre>
					) : null}
				</div>
			</details>
			...
			<details open={outputOpen} onToggle={(e) => setOutputOpen(e.currentTarget.open)} className="group">
				<summary className="px-3 py-1 text-[11px] text-gray-400 dark:text-gray-500 cursor-pointer hover:text-gray-600 dark:hover:text-gray-400 select-none flex items-center gap-1.5 border-t border-gray-200 dark:border-gray-700/30">
					<svg className="w-3 h-3 transition-transform group-open:rotate-90 shrink-0" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4.5 3l3 3-3 3" /></svg>
					<span>Output</span>
					{isRunning && <span className="ml-auto text-blue-500/70 dark:text-blue-400/70 animate-pulse text-[10px]">streaming</span>}
				</summary>
				<div className="px-3 pb-2">
					{block.output ? (
						<AnsiText
