...
const mockedSessionGetState = vi.mocked(useSessionStore.getState)
...
describe("checkPermissionClear", () => {
	...
	it("does not update status when other pending requests remain for same session", () => {
		...
		useUIDialogStore.getState().registerUIRequest(makeRequest({ requestId: "r1", sessionId: "sess-1" }))
		useUIDialogStore.getState().registerUIRequest(makeRequest({ requestId: "r2", sessionId: "sess-1" }))
