...
describe("addEvent", () => {
	it("adds event to correct session", () => {
		const event = { id: "e1", customType: "memory.write", data: { foo: "bar" }, timestamp: Date.now() }
...
describe("loadFiles", () => {
	it("updates files and entrypoint on success", async () => {
		const files = [
			{ filename: "pref.md", filePath: "/a/pref.md", description: "desc", type: "user", mtimeMs: 1000, size: 50 },
		]
...
describe("multiple sessions coexist independently", () => {
	it("events/files/injected for session A do not leak to session B", async () => {
		useMemoryStore.getState().addEvent("sess-a", { id: "ea", customType: "memory_extract", data: null, timestamp: 1 })
		useMemoryStore.getState().addInjected("sess-a", { summary: "a-only", snippet: "snip" })
		mockedCall.mockResolvedValueOnce({
			files: [{ filename: "a.md", filePath: "/a.md", description: "a file", type: "user", mtimeMs: 1, size: 10 }],
			entrypointContent: null,
		})
		await useMemoryStore.getState().loadFiles("/project-a", "sess-a")

		useMemoryStore.getState().addEvent("sess-b", { id: "eb", customType: "memory_prefetch", data: null, timestamp: 2 })
		mockedCall.mockResolvedValueOnce({
			files: [{ filename: "b.md", filePath: "/b.md", description: "b file", type: "feedback", mtimeMs: 2, size: 20 }],
...
describe("addEvent with large data", () => {
	it("handles very large data without crash", () => {
		...
		const state = useMemoryStore.getState()
		expect(state.eventsBySession["sess-1"]).toHaveLength(1)
		expect((state.eventsBySession["sess-1"][0].data as { content: string }).content).toHaveLength(10 * 1024)
	})
})
...
describe("clearSession", () => {
	it("removes all data for a session", () => {
		useMemoryStore.getState().addEvent("sess-1", { id: "e1", customType: "a", data: null, timestamp: 1 })
	...
	it("does not affect other sessions", () => {
		useMemoryStore.getState().addEvent("sess-1", { id: "e1", customType: "a", data: null, timestamp: 1 })
		useMemoryStore.getState().addEvent("sess-2", { id: "e2", customType: "b", data: null, timestamp: 2 })
