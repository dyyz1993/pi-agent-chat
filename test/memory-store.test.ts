...
beforeEach(() => {
  vi.clearAllMocks();
  // Reset session store to avoid state leakage from other tests
  (useSessionStore as unknown as { setState: (patch: Record<string, unknown>) => void }).setState({
    activeSessionId: "test-session",
    sessionReady: { "test-session": true },
    sessionStatusMap: {},
    restoreContextFromHistory: vi.fn(),
  });
  // Reset memory store using zustand's setState
  (useMemoryStore as unknown as { setState: (state: unknown) => void }).setState({
    eventsBySession: {},
    filesBySession: {},
    entrypointBySession: {},
    injectedBySession: {},
